					...
					if (permission_exists('xml_cdr_recording') && (permission_exists('xml_cdr_recording_play') || permission_exists('xml_cdr_recording_download'))) {
						if ($record_path != '') {
							...
							if (permission_exists('xml_cdr_recording_download')) {
								if (substr($record_path, 0, 22) == "/mnt/azure_recordings/") {
									$content .= button::create(['type'=>'button','title'=>$text['label-download'],'icon'=>$_SESSION['theme']['button_icon_download'],'link'=>"https://f003.backblazeb2.com/file/recordings-azure/".substr($record_path, 22)."/".urlencode($row['xml_cdr_uuid']).".mp3"]);
								} else {
